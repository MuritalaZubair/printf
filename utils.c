...
{
	/* Check if character is within printable ASCII range */
	if (c >= 32 && c < 127)
		return (1);
...
int append_hexa_code(char ascii_code, char buffer[], int i)
{
	...
	/* Convert negative ASCII code to positive */
	if (ascii_code < 0)
		ascii_code *= -1;

	/* Append the escape character and 'x' to the buffer */
	buffer[i++] = '\\';
	buffer[i++] = 'x';

	/* Append the hexadecimal representation to the buffer */
